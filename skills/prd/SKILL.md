...
5. **Decompose into Stories.** Each Story must satisfy every rule in `story-readiness.md` and be credible for one `gpt-5.6-luna` high-reasoning Ralph Iteration. Dependencies may point only to earlier Stories. Split independent outcomes, conditional work, and oversized changes; do not copy source headings mechanically. Give each Story one primary implementation seam and one focused verification target.
6. **Audit Story size adversarially.** For every draft Story, partition its acceptance criteria into behavior clusters that could fail independently. Split clusters that exercise separate lifecycle paths, state machines, caller migrations, or test matrices. A Story with more than eight atomic acceptance criteria must be split unless the outcome is genuinely indivisible; when it is indivisible, explain that constraint to the user instead of silently accepting the size. Review adjacent Stories for overlapping completion criteria, and assign each behavior to the earliest Story that can leave the repository coherent. Then reread each Story as the Luna Iteration: it must execute settled decisions rather than rediscover how responsibilities should be divided. Completion: every Story has one observable completion boundary and no independently deliverable behavior cluster remains inside it.
